...
Отдельной сущности «папка» нет — есть префикс в пути файла. Поэтому операция
над папкой разворачивается на сервере в операции над каждым её файлом (включая
вложенные) и попадает в журнал операций как набор `MOVE`/`DELETE`. Делать то же
на клиенте означало бы сотни запросов и частичный результат при первом сбое.

| Метод  | Путь                                        | Описание                                                                                                                                                                                                                            |
| ------ | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/api/projects/[id]/folders/download?path=` | Папка одним `.zip`.                                                                                                                                                                                                                 |
| PATCH  | `/api/projects/[id]/folders`                | `{ path, newPath }` — переименование/перенос. Все целевые пути проверяются **до** первого переноса: занятый путь → `409 path_exists`, файл на месте будущего каталога → `409 path_blocked`, папка внутрь себя → `400 invalid_path`. |
| DELETE | `/api/projects/[id]/folders?path=`          | Soft delete всех файлов папки. Ответ `{ path, deleted }`.                                                                                                                                                                           |
