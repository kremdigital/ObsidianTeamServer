...
| PATCH  | `/api/projects/[id]/folders`                | `{ path, newPath }` — переименование/перенос. Все целевые пути проверяются **до** первого переноса: занятый путь → `409 path_exists`, папка внутрь себя → `400 invalid_path`. |
